...
- Component work: read [component assets](references/component-assets.md), search the local registry first, then acquire only an exact confirmed MPN. For official product photos, use the discover → capture → human-confirmed attach flow; never scrape arbitrary search-result images or reuse touchpoints after an appearance changes. For a Rune-quality wireable top view, read [interactive board views](references/board-views.md) and generate an original SVG from explicit manufacturer DXF/footprint/pinout coordinates; never infer electrical identity from pixels.
